	...
	private void paint() {
		...
		try {
			...
			try {
				switch (Platform.get()) {
				case MACOSX:
					throw new UnsupportedOperationException("macOS not supported.");
				case LINUX:
					...
					if (context == NULL) {
						...
						if (!glXMakeCurrent(dsi_x11.display(), drawable, context)) { throw new IllegalStateException("glXMakeCurrent() failed"); }
				...
				case WINDOWS:
					...
					long hdc = dsi_win.hdc(); // hdc changes!!!!!!
					if (hdc == NULL) {
						break;
					}
